...
abstract class Todopago_Modulodepago2_Model_Cybersource_Cybersource extends Mage_Core_Model_Abstract{
	...
	private function completeCS(){
		...
		$payDataOperacion ['CSBTFIRSTNAME'] = Mage::helper('modulodepago2/data')->removeSpecialChars($this->getField($billingAdress->getFirstname()));
 		$payDataOperacion ['CSBTLASTNAME'] = Mage::helper('modulodepago2/data')->removeSpecialChars($this->getField($billingAdress->getLastname()));
		...
		if(!$payDataOperacion ['CSBTPHONENUMBER'] || strlen($payDataOperacion ['CSBTPHONENUMBER']) < 6) $payDataOperacion ['CSBTPHONENUMBER'] = '111111111';
