...
class Todopago_Modulodepago2_Adminhtml_CredentialController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function productionAction()
    {
		if($this->getRequest()->getPost('user') != null) {
			...
			if(count($this->_error) == 0) {
				...
				echo '<script>window.opener.document.getElementById("payment_todopago_modo_idstore").value="'.$user->getMerchant().'";</script>';
				echo '<script>window.opener.document.getElementById("payment_todopago_modo_codigo_seguridad").value="'.$sec[1].'";</script>';
				echo '<script>window.opener.document.getElementById("payment_todopago_modo_apikey").value="'.$user->getApikey().'";</script>';
    ...
    public function developersAction()
    {
		if($this->getRequest()->getPost('user') != null) {
			...
			$sec = explode(" ",$user->getApikey());
			if(count($this->_error) == 0) {
				echo '<script>window.opener.document.getElementById("payment_todopago_modo_idstore_test").value="'.$user->getMerchant().'";</script>';
				echo '<script>window.opener.document.getElementById("payment_todopago_modo_codigo_seguridad_test").value="'.$sec[1].'";</script>';
				echo '<script>window.opener.document.getElementById("payment_todopago_modo_apikey_test").value="'.$user->getApikey().'";</script>';
				echo '<script>window.close();</script>';
				die;
			}
		}
